...
新增功能：
- 另存 prob_fake：--out_prob_csv 會輸出 (filename, prob_fake)
- 仍保留原本 submission：--out_csv 輸出 (filename, label)

輸出：
1) out_csv: filename,label   label ∈ {real,fake}
2) out_prob_csv(可選): filename,prob_fake   prob_fake ∈ [0,1]
"""
...
def main():
    ...
    ap.add_argument("--out_csv", type=str, default="submission_final.csv",
                    help="輸出 (filename,label)")
    ap.add_argument("--out_prob_csv", type=str, default="",
                    help="若提供路徑，額外輸出 (filename,prob_fake)")
    ...
    ap.add_argument("--threshold", type=float, default=0.5, help="輸出 fake 的門檻（prob_fake）")
    ...
    head = LinearHead(dim) if args.head == "linear" else MLPHead(dim, hidden=args.hidden, dropout=args.dropout)
    head = head.to(device)
    ...
    # (A) optional prob csv
    if args.out_prob_csv:
        prob_df = pd.DataFrame({"filename": names, "prob_fake": prob_fake})
        prob_df.to_csv(args.out_prob_csv, index=False)
        print(f"[OK] Saved prob csv: {args.out_prob_csv}")

    # (B) submission csv
    sub = pd.DataFrame({
        "filename": names,
        "label": np.where(prob_fake >= float(args.threshold), "fake", "real")
    })
    sub.to_csv(args.out_csv, index=False)

    fake_ratio = float(np.mean(sub["label"].values == "fake"))
    print(f"[OK] Saved {args.out_csv} | fake_ratio={fake_ratio:.3f} | best_val_auc={best_auc:.4f}")
